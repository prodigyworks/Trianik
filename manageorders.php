...
<?php
		...
		public function postUpdateEvent($orderid) {
			$items = json_decode($_POST['item_serial'], true);
			$memberid = getLoggedOnMemberID();
			
			$qry = "DELETE FROM {$_SESSION['DB_PREFIX']}orderitem 
			...
			foreach ($items as $k=>$item) {
				...
				$qry = "INSERT INTO {$_SESSION['DB_PREFIX']}orderitem 
						(orderid, sequence, quantity, priceeach, vatrate, vat, linetotal, 
						productid, metacreateddate, metacreateduserid, metamodifieddate, metamodifieduserid) 
						VALUES 
						($orderid, $sequence, '$qty', '$unitprice', $vatrate, '$vat', $linetotal, 
...
?>
			...
			callAjax(
					"finddata.php", 
					{ 
						sql: "SELECT A.*, B.description FROM <?php echo $_SESSION['DB_PREFIX'];?>orderitem A LEFT OUTER JOIN <?php echo $_SESSION['DB_PREFIX'];?>product B ON B.id = A.productid WHERE A.orderid = " + currentCrudID + " ORDER BY A.sequence"
					},
					function(data) {
						itemArray = data;
